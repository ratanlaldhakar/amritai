-- ============================================================
-- AMRIT YOGA CENTER — SHARED DATABASE SCHEMA MIGRATION
-- Run this in your Supabase SQL Editor for kxsovgiapzbpxahtkbuo
-- ============================================================

-- 1. Create students table if it doesn't exist on the Website project
...
-- 2. Create leads table with CRM columns
CREATE TABLE IF NOT EXISTS public.leads (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    phone_number TEXT UNIQUE NOT NULL, -- Canonical E.164 format
    name TEXT,
    interest TEXT,                     -- e.g. 'Yoga Therapy', 'Advanced Batch'
    preferred_batch TEXT,              -- e.g. '6:00 AM Therapy'
    goal TEXT,                         -- e.g. 'Weight Loss', 'Cure Back Pain'
    source TEXT,                       -- e.g. 'Website', 'WhatsApp Direct', 'CSV Import'
    assigned_to UUID REFERENCES public.admin_profiles(id) ON DELETE SET NULL,
    status TEXT CHECK (status IN ('new', 'contacted', 'trial_booked', 'enrolled', 'lost')) DEFAULT 'new',
    follow_up_date TIMESTAMPTZ,
    last_contact_at TIMESTAMPTZ DEFAULT NOW(),
    city TEXT,
    health_notes TEXT,                 -- Therapeutic conditions/joint pains
    notes TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- 3. Create conversation_threads table
CREATE TABLE IF NOT EXISTS public.conversation_threads (
    ...
    last_ai_response TEXT,             -- Cached last bot message
    last_human_reply TEXT,             -- Cached last admin message
    ai_enabled BOOLEAN DEFAULT true NOT NULL,
    handover_reason TEXT,              -- Reason for low confidence handover
    status TEXT CHECK (status IN ('bot_handling', 'live_chat', 'human_required')) DEFAULT 'bot_handling',
    assigned_to UUID REFERENCES public.admin_profiles(id) ON DELETE SET NULL,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- 4. Create messages table for WhatsApp logs
CREATE TABLE IF NOT EXISTS public.messages (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    thread_id UUID REFERENCES public.conversation_threads(id) ON DELETE CASCADE,
    message_id TEXT UNIQUE NOT NULL,   -- WhatsApp message ID (wamid)
...
-- 5. Create knowledge_base table for AI FAQs
CREATE TABLE IF NOT EXISTS public.knowledge_base (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    category TEXT NOT NULL,
    question TEXT NOT NULL,
    answer TEXT NOT NULL,              -- Rich Markdown Content
    keywords TEXT[],                   -- Match terms
    tags TEXT[],                       -- Label tags
    language TEXT DEFAULT 'en' NOT NULL, -- 'en', 'hi', or 'hinglish'
...
CREATE INDEX IF NOT EXISTS idx_kb_priority ON public.knowledge_base(priority DESC);

-- 6. Create settings table for dynamic AI settings
...
-- 8. Authenticated policies (Allows admin API access)
CREATE POLICY admin_all_students ON public.students TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY admin_all_leads ON public.leads TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY admin_all_threads ON public.conversation_threads TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY admin_all_messages ON public.messages TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY admin_all_kb ON public.knowledge_base TO authenticated USING (true) WITH CHECK (true);
CREATE POLICY admin_all_settings ON public.settings TO authenticated USING (true) WITH CHECK (true);

-- 9. Public Read Policy for Knowledge Base (for AI and website widgets)
CREATE POLICY public_read_kb ON public.knowledge_base FOR SELECT USING (is_published = true);

-- 10. Seed Bhilwara Center verified knowledge into public.knowledge_base
INSERT INTO public.knowledge_base (category, question, answer, language, priority) VALUES
(
  'location',
